let dados = [
  {
    nome: "CHARLES LECLERC",
    foto: 'drivers-image/charles-lecler.png',
    nacionalidade: "Monegásco",
    equipe: "Scuderia Ferrari",
    descricao:
      "Charles Leclerc, nascido em Mônaco, é um dos pilotos mais promissores da atual geração da Fórmula 1. Conhecido por sua habilidade, velocidade e determinação, Leclerc conquistou o coração dos fãs da Ferrari ao demonstrar um talento excepcional desde sua estreia na categoria. Sua ascensão meteórica, marcada por vitórias memoráveis e duelos acirrados com os principais adversários, o posiciona como um dos favoritos para conquistar títulos mundiais nos próximos anos.",
    link: "https://pt.wikipedia.org/wiki/Charles_Leclerc",
  },
  {
    nome: "LANDO NORRIS",
    foto: 'drivers-image/lando-norris.png',
    nacionalidade: "Britânico",
    equipe: "McLaren",
    descricao:
      "Lando Norris, um jovem talento britânico que vem se destacando na Fórmula 1. Conhecido por seu estilo de pilotagem agressivo e carisma, Norris conquistou a admiração dos fãs da McLaren desde sua estreia. Sua juventude e habilidade o colocam como um dos pilotos mais promissores da nova geração. Com um futuro promissor pela frente, Norris é considerado um dos principais candidatos a conquistar títulos mundiais nos próximos anos.",
    link: "https://pt.wikipedia.org/wiki/Lando_Norris",
  },
  {
    nome: "LEWIS HAMILTON",
    foto: 'drivers-image/lewis-hamilton.png',
    nacionalidade: "Britânico",
    equipe: "Mercedes",
    descricao:
      "Lewis Hamilton, um ícone da Fórmula 1. Lewis Hamilton é um dos maiores pilotos da história da categoria. Com múltiplos títulos mundiais e um talento inegável, Hamilton se tornou um símbolo do esporte. Sua rivalidade com outros grandes pilotos, como Sebastian Vettel e Max Verstappen, marcou época na Fórmula 1. Sua habilidade no volante, combinada com sua personalidade carismática e engajamento em causas sociais, fazem de Hamilton uma figura inspiradora para milhões de fãs ao redor do mundo.",
    link: "https://pt.wikipedia.org/wiki/Lewis_Hamilton",
  },
  {
    nome: "MAX VERSTAPPEN",
    foto: 'drivers-image/max-verstappen.png',
    nacionalidade: "Holandesa",
    equipe: "Red Bull",
    descricao:
      "Max Verstappen, o piloto holandês, emergiu como uma das maiores forças da Fórmula 1 na última década. Com um estilo de pilotagem agressivo e uma habilidade impressionante de ultrapassagem, Verstappen conquistou o coração dos fãs e se tornou um dos nomes mais comentados do esporte. Desde sua estreia precoce na categoria, o holandês demonstrou um talento natural e uma maturidade incomum para sua idade, o que o levou a conquistar títulos mundiais e a estabelecer diversos recordes. Sua rivalidade com Lewis Hamilton, que culminou em um emocionante campeonato de 2021, consolidou seu status como um dos maiores talentos da história da Fórmula 1",
    link: "https://pt.wikipedia.org/wiki/Max_Verstappen",
  },
  {
    nome: "CARLOS SAINZ JR",
    foto: 'drivers-image/carlos-sainz.png',
    nacionalidade: "Espanhola",
    equipe: "Scuderia Ferrari",
    descricao:
      "Carlos Sainz Jr., filho do lendário campeão mundial de rali Carlos Sainz, é um piloto espanhol que se destaca na Fórmula 1 por sua consistência, habilidade e sangue frio. Desde sua estreia na categoria, Sainz tem impressionado pela sua capacidade de extrair o máximo de seus carros e por suas batalhas acirradas na pista. Com uma carreira sólida e em constante evolução, o espanhol se tornou um dos pilotos mais queridos e respeitados do grid, sendo um dos pilares da Scuderia Ferrari. Sua experiência, aliada ao seu talento natural, o coloca como um forte candidato a conquistar títulos mundiais nos próximos anos.",
    link: "https://pt.wikipedia.org/wiki/Carlos_Sainz",
  },
  {
    nome: "GEORGE RUSSELL",
    foto: 'drivers-image/george-russell.png',
    nacionalidade: "Britânico",
    equipe: "Mercedes",
    descricao:
      "George Russell, piloto britânico da Mercedes, tem se destacado como uma das maiores promessas da nova geração da Fórmula 1. Desde sua estreia, tem impressionado pela sua consistência, habilidade e maturidade. Considerado um dos pilotos mais rápidos e talentosos do grid, Russell já conquistou uma vitória na Fórmula 1 e tem demonstrado um grande potencial para alcançar ainda mais sucesso na categoria. Sua ascensão meteórica, aliada à sua parceria com Lewis Hamilton, promete um futuro brilhante para o piloto britânico.",
    link: "https://pt.wikipedia.org/wiki/George_Russell",
  },
  {
    nome: "SERGIO PÉREZ",
    foto: 'drivers-image/sergio-perez.png',
    nacionalidade: "Mexicana",
    equipe: "Red Bull",
    descricao:
      "Sergio Pérez, conhecido carinhosamente como 'Checo Pérez', é um piloto mexicano que se destaca na Fórmula 1 por sua experiência e consistência. Com uma longa trajetória na categoria, Pérez é conhecido por sua habilidade em extrair o máximo de seus carros e por sua capacidade de trabalhar em equipe. O mexicano tem se mostrado um forte aliado para a Red Bull, contribuindo para os sucessos da equipe nas últimas temporadas. Sua habilidade em ultrapassagens e em condições de corrida desafiadoras o tornaram um dos pilotos mais respeitados do grid.",
    link: "https://pt.wikipedia.org/wiki/Sergio_Pérez",
  },
  {
    nome: "OSCAR PIASTRI",
    foto: 'drivers-image/oscar-piastri.png',
    nacionalidade: "Australiana",
    equipe: "McLaren",
    descricao:
      "Oscar Piastri, o jovem australiano, representa uma nova geração de pilotos na Fórmula 1. Campeão da Fórmula 2 em 2021, Piastri chegou à categoria máxima do automobilismo com grande expectativa, e até agora tem mostrado um talento promissor. Sua adaptação ao carro da McLaren tem sido rápida, e o australiano já demonstra um estilo de pilotagem preciso e consistente. Com um futuro brilhante pela frente, Piastri é um dos nomes a se acompanhar de perto nos próximos anos.",
    link: "https://pt.wikipedia.org/wiki/Oscar_Piastri",
  },
  {
    nome: "PIERRE GASLY",
    foto: 'drivers-image/pierre-gasly.png',
    nacionalidade: "Francês",
    equipe: "Alpine",
    descricao:
      "Pierre Gasly, piloto francês com uma trajetória sólida na Fórmula 1. Conhecido por sua habilidade de ultrapassagem e adaptabilidade, Gasly já conquistou um pódio na Fórmula 1 e demonstrou ser um piloto rápido e consistente. Sua experiência e maturidade o tornam um forte competidor no grid. Com uma carreira em constante evolução, Gasly busca consolidar seu lugar entre os melhores pilotos da categoria.",
    link: "https://pt.wikipedia.org/wiki/Pierre_Gasly",
  },
  {
    nome: "ESTEBAN OCON",
    foto: 'drivers-image/esteban-ocon.png',
    nacionalidade: "Francês",
    equipe: "Alpine",
    descricao:
      "Esteban Ocon, piloto francês com uma carreira promissora na Fórmula 1. Conhecido por sua técnica apurada e consistência, Ocon já conquistou um pódio na Fórmula 1 e demonstrou ser um piloto rápido e estratégico. Sua experiência na equipe reserva o torna um forte aliado para o desenvolvimento do carro. Com uma carreira em constante ascensão, Ocon busca conquistar títulos mundiais e consolidar seu lugar entre os grandes nomes da Fórmula 1.",
    link: "https://pt.wikipedia.org/wiki/Esteban_Ocon",
  },
  {
    nome: "DANIEL RICCIARDO",
    foto: 'drivers-image/daniel-ricciardo.png',
    nacionalidade: "Australiano",
    equipe: "Racing Bulls",
    descricao:
      "Daniel Ricciardo, piloto australiano conhecido por seu estilo de pilotagem agressivo e carismático. Vencedor de diversas corridas na Fórmula 1, Ricciardo é um dos pilotos mais populares do grid. Sua experiência e velocidade o tornam um forte candidato a pódios e vitórias.",
    link: "https://pt.wikipedia.org/wiki/Daniel_Ricciardo",
  },
  {
    nome: "YUKI TSUNODA",
    foto: 'drivers-image/yuki-tsunoda.png',
    nacionalidade: "Japonês",
    equipe: "Racing Bulls",
    descricao:
      "Yuki Tsunoda, jovem piloto japonês que está despontando na Fórmula 1. Conhecido por sua velocidade e agressividade, Tsunoda tem mostrado grande potencial e é considerado uma das promessas da categoria. Sua adaptação à Fórmula 1 tem sido rápida e impressionante.",
    link: "https://pt.wikipedia.org/wiki/Yuki_Tsunoda",
  },
  {
    nome: "VALTTERI BOTTAS",
    foto: 'drivers-image/valtteri-bottas.png',
    nacionalidade: "Finlandês",
    equipe: "Kick Sauber",
    descricao:
      "Valtteri Bottas, piloto finlandês com vasta experiência na Fórmula 1. Campeão de diversas corridas, Bottas é conhecido por sua consistência e habilidade em ultrapassagens. Sua experiência e velocidade o tornam um forte aliado para o desenvolvimento do carro.",
    link: "https://pt.wikipedia.org/wiki/Valtteri_Bottas",
  },
  {
    nome: "KEVIN MAGNUSSEN",
    foto: 'drivers-image/kevin-magnussen.png',
    nacionalidade: "Dinamarquês",
    equipe: "Haas",
    descricao:
      "Kevin Magnussen, piloto dinamarquês com experiência na Fórmula 1. Conhecido por sua agressividade e habilidade em ultrapassagens, Magnussen é um piloto rápido e experiente. Sua volta à Fórmula 1 tem sido marcada por resultados consistentes.",
    link: "https://pt.wikipedia.org/wiki.Kevin_Magnussen",
  },
  {
    nome: "NICO HULKENBERG",
    foto: 'drivers-image/nico-hulkenberg.png',
    nacionalidade: "Alemão",
    equipe: "Haas",
    descricao:
      "Nico Hulkenberg, piloto alemão com vasta experiência na Fórmula 1. Conhecido por sua consistência e habilidade em qualificação, Hulkenberg é um piloto rápido e experiente. Sua volta à Fórmula 1 tem sido marcada por resultados consistentes.",
    link: "https://pt.wikipedia.org/wiki/Nico_Hulkenberg",
  },
  {
    nome: "ZHOU GUANYU",
    foto: 'drivers-image/zhou-guanyu.png',
    nacionalidade: "Chinês",
    equipe: "Kick Sauber",
    descricao: "Zhou Guanyu, o primeiro piloto chinês a competir na Fórmula 1, tem impressionado pela sua adaptação rápida à categoria. Conhecido por sua calma e consistência, Zhou busca se consolidar como um dos pilotos mais promissores do grid.",
    link: "https://pt.wikipedia.org/wiki/Zhou_Guanyu",
},
{
    nome: "ALEXANDER ALBON",
    foto: 'drivers-image/alex-albon.png',
    nacionalidade: "Tailandês",
    equipe: "Williams",
    descricao: "Alexander Albon, piloto tailandês com experiência em equipes de ponta, retorna à Fórmula 1 com a Williams. Conhecido por sua habilidade em ultrapassagens e ritmo de corrida, Albon busca levar a Williams a novos patamares.",
    link: "https://pt.wikipedia.org/wiki/Alexander_Albon",
},
{
    nome: "LANCE STROLL",
    foto: 'drivers-image/lance-stroll.png',
    nacionalidade: "Canadense",
    equipe: "Aston Martin",
    descricao: "Lance Stroll, filho do proprietário da Aston Martin, tem se mostrado um piloto consistente na Fórmula 1. Com experiência e desenvolvimento, Stroll busca alcançar resultados ainda mais expressivos e contribuir para o crescimento da equipe.",
    link: "https://pt.wikipedia.org/wiki/Lance_Stroll",
},
{
    nome: "FERNANDO ALONSO",
    foto: 'drivers-image/fernando-alonso.png',
    nacionalidade: "Espanhol",
    equipe: "Aston Martin",
    descricao: "Fernando Alonso, um dos pilotos mais experientes e vitoriosos da Fórmula 1, retornou à categoria após uma breve pausa. Conhecido por sua paixão e habilidade, Alonso busca mais títulos mundiais e inspirar uma nova geração de pilotos.",
    link: "https://pt.wikipedia.org/wiki/Fernando_Alonso",
},
{
    nome: "LOGAN SARGEANT",
    foto: 'drivers-image/logan-sargeant.png',
    nacionalidade: "Americano",
    equipe: "Williams",
    descricao: "Logan Sargeant, o primeiro piloto americano a competir na Fórmula 1 em muitos anos, é uma das grandes promessas da nova geração. Com talento e determinação, Sargeant busca se destacar na categoria e representar os Estados Unidos no mais alto nível do automobilismo.",
    link: "https://pt.wikipedia.org/wiki/Logan_Sargeant",
}
];
